...
func (l *LeafNode) Put(key []byte, value []byte) error {
	idx, found := slices.BinarySearchFunc(l.key, key, func(a, b []byte) int {
		return bytes.Compare(a, b)
	})

	if found {
		if bytes.Equal(l.value[idx], value) {
	...
	l.key = slices.Insert(l.key, idx, key)
	l.value = slices.Insert(l.value, idx, value)
	return nil
	// length := len(l.key)
...
func (l *LeafNode) Delete(key []byte) bool {
	...
	l.key = slices.Delete(l.key, idx, idx+1)
	l.value = slices.Delete(l.value, idx, idx+1)
	return true
	// length := len(l.key)
